...
public class JwtService {
	...
	private long refreshTokenExpirationTimeInMs = 3600000;

	private long accessTokenExpirationTimeInMs = 1800000;

	private long resetPasswordExpirationTimeInMs = 86400000;

	private long enableAccountExpirationTimeInMs = 259200000;
	 ...
	 * <p>
	 * The token contains the user's username as the subject, the current time as
	 * the issued at date
	 */
	...
	public String generateToken(String username, long expirationTimeInMs) {
		...
		return Jwts.builder().setSubject(username).setIssuedAt(ISSUED_AT).setExpiration(expirationDate) // set
																										// expiration
																										// date
	...
	public String generateAccessToken(User user) {
		...
		return Jwts.builder().setClaims(getUserClaims(user)) // add user claims
				.setSubject(user.getEmail()).setIssuedAt(ISSUED_AT).setExpiration(accessTokenExpirationDate)
	...
	/**
	 * Retrieves the key to be used to sign and validate JWT tokens based on the
